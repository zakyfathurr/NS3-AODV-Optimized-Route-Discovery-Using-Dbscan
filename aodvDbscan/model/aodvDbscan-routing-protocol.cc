...
//     }                                                           
//   } while (false)
#include "aodvDbscan-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("aodvDbscanRoutingProtocol");

namespace aodvDbscan {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for aodvDbscan control traffic
const uint32_t RoutingProtocol::aodvDbscan_PORT = 654;

/**
* \ingroup aodvDbscan
* \brief Tag used by aodvDbscan implementation
*/
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::aodvDbscan::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("aodvDbscan")
...
{
  static TypeId tid = TypeId ("ns3::aodvDbscan::RoutingProtocol")
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
  *stream->GetStream () << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
                        ...
                        << ", aodvDbscan Routing table" << std::endl;
                              ...
                              Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
  ...
  if (!p)
    {
      NS_LOG_DEBUG ("Packet is == nullptr");
      return LoopbackRoute (header, oif); // later
    }
  if (m_socketAddresses.empty ())
    {
      sockerr = Socket::ERROR_NOROUTETOHOST;
      NS_LOG_LOGIC ("No aodvDbscan interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                              ...
                              const ErrorCallback& ecb)
{
  ...
  // aodvDbscan is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
              ...
              if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
                {
                  ...
                  if (udpHeader.GetDestinationPort () == aodvDbscan_PORT)
                    {
                      // aodvDbscan packets sent in broadcast are already managed
                      return true;
                    }
                }
              if (header.GetTtl () > 1)
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (l3->GetNAddresses (i) > 1)
    {
      NS_LOG_WARN ("aodvDbscan does not work with more then one address per each interface.");
    }
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvaodvDbscan, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), aodvDbscan_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvaodvDbscan, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), aodvDbscan_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 1)
    ...
    {
      NS_LOG_LOGIC ("aodvDbscan does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in aodvDbscan operation");
    }
}
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when aodvDbscan does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // aodvDbscan needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available aodvDbscan interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid aodvDbscan source address not found");
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  //std::cout << "Sending...\n";
  // Send RREQ as subnet directed broadcast from each interface used by aodvDbscan
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (aodvDbscanTYPE_RREQ);
      ...
      else 
      {
        ...
        else
        {
            ...
            selectedCluster = m_routingTable.DBSCAN(dst, posDst.first, posDst.second,0.3,2);
...
{
  socket->SendTo (packet, 0, InetSocketAddress (destination, aodvDbscan_PORT));
...
RoutingProtocol::ScheduleRreqRetry (Ipv4Address dst)
{
  ...
  // ✅ FIX: Ensure retry time is always positive
  if (retry.IsNegative() || retry.IsZero())
  {
    NS_LOG_WARN("Calculated retry time is non-positive (" << retry.As(Time::MS) 
                << " ms), using default 100ms");
    retry = MilliSeconds(100);
  }
  
  // ✅ FIX: Add safety check for very large delays
  Time maxRetry = Seconds(10);
  if (retry > maxRetry)
  {
    NS_LOG_WARN("Retry time too large (" << retry.As(Time::S) 
                << " s), capping at " << maxRetry.As(Time::S) << " s");
    retry = maxRetry;
  }
...
void
RoutingProtocol::RecvaodvDbscan (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("aodvDbscan node " << this << " received a aodvDbscan packet from " << sender << " to " << receiver);
  ...
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("aodvDbscan message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
      return; // drop
    }
  switch (tHeader.Get ())
    {
    case aodvDbscanTYPE_RREQ:
      {
        RecvRequest (packet, receiver, sender);
        break;
      }
    case aodvDbscanTYPE_RREP:
      {
        RecvReply (packet, receiver, sender);
        break;
      }
    case aodvDbscanTYPE_RERR:
      {
        RecvError (packet, sender);
        break;
      }
    case aodvDbscanTYPE_RREP_ACK:
      {
        RecvReplyAck (sender);
        break;
      }
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  TypeHeader tHeader (aodvDbscanTYPE_RREP);
  packet->AddHeader (tHeader);
  Ptr<Socket> socket = FindSocketWithInterfaceAddress (toOrigin.GetInterface ());
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (toOrigin.GetNextHop (), aodvDbscan_PORT));
}
  ...
  if (gratRep)
    {
      ...
      TypeHeader type (aodvDbscanTYPE_RREP);
      packetToDst->AddHeader (type);
      Ptr<Socket> socket = FindSocketWithInterfaceAddress (toDst.GetInterface ());
      NS_ASSERT (socket);
      NS_LOG_LOGIC ("Send gratuitous RREP " << packet->GetUid ());
      socket->SendTo (packetToDst, 0, InetSocketAddress (toDst.GetNextHop (), aodvDbscan_PORT));
    }
}
...
RoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
  ...
  RrepAckHeader h;
  TypeHeader typeHeader (aodvDbscanTYPE_RREP_ACK);
  Ptr<Packet> packet = Create<Packet> ();
  SocketIpTtlTag tag;
  tag.SetTtl (1);
  packet->AddPacketTag (tag);
  ...
  NS_ASSERT (socket);
  socket->SendTo (packet, 0, InetSocketAddress (neighbor, aodvDbscan_PORT));
}
  ...
  for (std::map<Ipv4Address, uint32_t>::const_iterator i = unreachable.begin ();
       i != unreachable.end (); )
    {
      if (!rerrHeader.AddUnDestination (i->first, i->second))
        {
          TypeHeader typeHeader (aodvDbscanTYPE_RERR);
          Ptr<Packet> packet = Create<Packet> ();
          SocketIpTtlTag tag;
          tag.SetTtl (1);
          packet->AddPacketTag (tag);
...
RoutingProtocol::SendPacketFromQueue (Ipv4Address dst, Ptr<Ipv4Route> route)
{
  ...
  // std::cout << "Best forwarder by aodvDbscan = " << bestForwarder << "\n";
                                               ...
                                               uint32_t dstSeqNo, Ipv4Address origin)
{
  ...
  packet->AddPacketTag (tag);
  packet->AddHeader (rerrHeader);
  packet->AddHeader (TypeHeader (aodvDbscanTYPE_RERR));
  if (m_routingTable.LookupValidRoute (origin, toOrigin))
    {
  ...
  else
    {
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
             m_socketAddresses.begin (); i != m_socketAddresses.end (); ++i)
        {
          ...
          socket->SendTo (packet->Copy (), 0, InetSocketAddress (destination, aodvDbscan_PORT));
        }
    }
}
