 ...
 */
#ifndef aodvDbscanPACKET_H
#define aodvDbscanPACKET_H
...
enum MessageType
{
  aodvDbscanTYPE_RREQ  = 1,   //!< aodvDbscanTYPE_RREQ
  aodvDbscanTYPE_RREP  = 2,   //!< aodvDbscanTYPE_RREP
  aodvDbscanTYPE_RERR  = 3,   //!< aodvDbscanTYPE_RERR
  aodvDbscanTYPE_RREP_ACK = 4 //!< aodvDbscanTYPE_RREP_ACK
};

/**
* \ingroup aodvDbscan
* \brief aodvDbscan types
*/
...
public:
  /**
   * constructor
   * \param t the aodvDbscan RREQ type
   */
  TypeHeader (MessageType t = aodvDbscanTYPE_RREQ);
...
public:
  ...
  RrepHeader (uint8_t prefixSize = 0, uint8_t hopCount = 0, Ipv4Address dst =
                ...
                uint32_t txErrorCount = 0, uint32_t freeSpace = 0, uint32_t positionX=0, uint32_t positionY=0);
  /**
   * \brief Get the type ID.
   * \return the object TypeId
   */
  ...
  /**
   * \brief Set the txerror count
   * \param count the txerror count
   */
  void SetTxErrorCount (uint32_t count)
  {
    m_txErrorCount = count;
  }
  /**
   * \brief Get the txerror count
   * \return the txerror count
   */
  uint32_t GetTxErrorCount () const
  {
    return m_txErrorCount;
  }

    /**
   * \brief Set the free space
   * \param count the free space
   */
  void SetFreeSpace (uint32_t count)
  {
    m_freeSpace = count;
  }
  /**
...
private:
  ...
  uint32_t      m_txErrorCount;    ///< number of successful txerror by this node
  uint32_t      m_freeSpace;        ///< free space left in this node
  uint32_t      m_positionX;        ///< x position of node
  uint32_t      m_positionY;        ///< y position of node
