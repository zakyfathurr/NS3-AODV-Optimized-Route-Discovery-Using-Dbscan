 ...
 */
#ifndef aodvDbscanROUTINGPROTOCOL_H
#define aodvDbscanROUTINGPROTOCOL_H

#include "aodvDbscan-rtable.h"
#include "aodvDbscan-rqueue.h"
#include "aodvDbscan-packet.h"
#include "aodvDbscan-neighbor.h"
#include "aodvDbscan-dpd.h"
#include "ns3/node.h"
 ...
 * \ingroup aodvDbscan
 *
 * \brief aodvDbscan routing protocol
 */
...
public:
  ...
  static const uint32_t aodvDbscan_PORT;
...
private:
  ...
  /// interaction count with ip addresses
  std::map<Ipv4Address, uint32_t> m_interactionCount;
  /// successful delivery of messages
  uint32_t m_txerrorCount;
  /// my position
  Vector m_position;
  /// last known position of nodes
  std::map<Ipv4Address, std::pair<uint32_t, uint32_t> > m_lastKnownPosition;
  /// last known cluster
  std::map<Ipv4Address, std::vector<Ipv4Address>> m_lastKnonwCluster;
...
private:
   ...
   */
  void RecvaodvDbscan (Ptr<Socket> socket);
  /**
   * Receive RREQ
   * \param p packet
  ...
  /**
   * Select next forwarder of message
   * \param socket - destination node socket
   * \param packet - packet to send
   * \param destination - destination node IP address
   */
  void DBSCAN (Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination);
  ...
  /// clear cluster
  void ClusterTimerExpire(Ipv4Address dst);
  /**
...
#endif /* aodvDbscanROUTINGPROTOCOL_H */
