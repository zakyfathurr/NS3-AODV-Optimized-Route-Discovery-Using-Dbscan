...
#include "aodvDbscan-regression.h"
#include "bug-772.h"
...
#include "ns3/aodvDbscan-helper.h"
 ...
 * \ingroup tests
 *
 * \brief aodvDbscan regression test suite
 */
class aodvDbscanRegressionTestSuite : public TestSuite
{
public:
  aodvDbscanRegressionTestSuite () : TestSuite ("routing-aodvDbscan-regression", SYSTEM)
  {
    SetDataDir (NS_TEST_SOURCEDIR);
    // General RREQ-RREP-RRER test case
    AddTestCase (new ChainRegressionTest ("aodvDbscan-chain-regression-test"), TestCase::QUICK);
...
} g_aodvDbscanRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest (const char * const prefix, Time t, uint32_t size, Time arpAliveTimeout)
  : TestCase ("aodvDbscan chain regression test"),
    m_nodes (0),
    m_prefix (prefix),
    m_time (t),
    m_size (size),
...
ChainRegressionTest::CreateDevices ()
{
  ...
  // 2. Setup TCP/IP & aodvDbscan
  aodvDbscanHelper aodvDbscan; // Use default parameters here
  InternetStackHelper internetStack;
  internetStack.SetRoutingHelper (aodvDbscan);
  ...
  streamsUsed += aodvDbscan.AssignStreams (*m_nodes, streamsUsed);
  // aodvDbscan uses m_size more streams
