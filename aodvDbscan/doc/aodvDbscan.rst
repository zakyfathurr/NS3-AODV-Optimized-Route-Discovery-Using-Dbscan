...
Ad Hoc On-Demand Distance Vector (aodvDbscan)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (aodvDbscan) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 aodvDbscan model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the aodvDbscan-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the aodvDbscan model lives in the directory `src/aodvDbscan`.

Design
++++++

Class ``ns3::aodvDbscan::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::aodvDbscan::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::aodvDbscan::RouteInput``,
...
this functionality are attributes of ``ns3::aodvDbscan::RoutingProtocol``. 
...
aodvDbscan discovers routes on demand.  Therefore, the aodvDbscan model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in aodvDbscan-rqueue.cc. A smart pointer to 
...
* This aodvDbscan implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from aodvDbscan-UU implementation and can be disabled.
...
the assertion from the aodvDbscan RFC that aodvDbscan works over UDP.  This model uses
