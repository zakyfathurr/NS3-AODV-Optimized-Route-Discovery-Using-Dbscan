 ...
 */
#include "ns3/test.h"
#include "ns3/aodvDbscan-neighbor.h"
#include "ns3/aodvDbscan-packet.h"
#include "ns3/aodvDbscan-rqueue.h"
#include "ns3/aodvDbscan-rtable.h"
...
{
  TypeHeaderTest () : TestCase ("aodvDbscan TypeHeader")
  {
  }
  virtual void DoRun ()
  {
    TypeHeader h (aodvDbscanTYPE_RREQ);
    NS_TEST_EXPECT_MSG_EQ (h.IsValid (), true, "Default header is valid");
    NS_TEST_EXPECT_MSG_EQ (h.Get (), aodvDbscanTYPE_RREQ, "Default header is RREQ");

    Ptr<Packet> p = Create<Packet> ();
    p->AddHeader (h);
    TypeHeader h2 (aodvDbscanTYPE_RREP);
...
{
  RreqHeaderTest () : TestCase ("aodvDbscan RREQ")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepHeaderTest () : TestCase ("aodvDbscan RREP")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepAckHeaderTest () : TestCase ("aodvDbscan RREP-ACK")
  {
  }
  virtual void DoRun ()
  {
...
{
  RerrHeaderTest () : TestCase ("aodvDbscan RERR")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for aodvDbscan routing table entry
 */
...
struct aodvDbscanRqueueTest : public TestCase
{
  aodvDbscanRqueueTest () : TestCase ("Rqueue"),
                      q (64, Seconds (30))
  {
  }
  virtual void DoRun ();
...
aodvDbscanRqueueTest::DoRun ()
{
  ...
  h.SetDestination (Ipv4Address ("1.2.3.4"));
  h.SetSource (Ipv4Address ("4.3.2.1"));
  Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback (&aodvDbscanRqueueTest::Unicast, this);
  Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback (&aodvDbscanRqueueTest::Error, this);
  ...
  Simulator::Schedule (q.GetQueueTimeout () + Seconds (1), &aodvDbscanRqueueTest::CheckTimeout, this);
...
void
aodvDbscanRqueueTest::CheckSizeLimit ()
{
...
void
aodvDbscanRqueueTest::CheckTimeout ()
{
 ...
 * \brief Unit test for aodvDbscan routing table entry
 */
struct aodvDbscanRtableEntryTest : public TestCase
{
  aodvDbscanRtableEntryTest () : TestCase ("RtableEntry")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for aodvDbscan routing table
 */
struct aodvDbscanRtableTest : public TestCase
{
  aodvDbscanRtableTest () : TestCase ("Rtable")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief aodvDbscan test suite
 */
class aodvDbscanTestSuite : public TestSuite
{
public:
  aodvDbscanTestSuite () : TestSuite ("routing-aodvDbscan", UNIT)
  {
    ...
    AddTestCase (new aodvDbscanRqueueTest, TestCase::QUICK);
    AddTestCase (new aodvDbscanRtableEntryTest, TestCase::QUICK);
    AddTestCase (new aodvDbscanRtableTest, TestCase::QUICK);
  }
} g_aodvDbscanTestSuite; ///< the test suite
